...
cv::Point FindCircleCenter(EnergyResult DetectResult, int r, vector<Point2f>& circle_center)
{
	...
	if (DetectResult.flag == 1)
	{
		...
		if (circle_center.size() != 0 && GetDistance(circle_center[MinNumber], PreCircleCenter) < 80)
		{
...
int judge_direction(vector<float> CaliDirectionList)
{
	float num = 0;
	for (int i = 0; i < CaliDirectionList.size(); i++)
		num += CaliDirectionList[i];
	if (num > 0)
		return 1; //顺时针
	if (num < 0)
		return 2; //逆时针
...
void Image_Thread::Image_Thread_energy()
{
	...
	createTrackbar("Red_Brght", "Parameter", &Red_Brght, 200, NULL);
	createTrackbar("Blue_Color", "Parameter", &Blue_Color, 200, NULL);
	createTrackbar("Blue_Brght", "Parameter", &Blue_Brght, 200, NULL);
	createTrackbar("ElementSize ", "Parameter", &ElementSize, 10, NULL);
	createTrackbar("ElementSize1", "Parameter", &ElementSize1, 10, NULL);
	createTrackbar("ElementSize2", "Parameter", &ElementSize2, 10, NULL);
	...
	while (true)
	{
		...
		ee.EnergyProcess(src, mask_bright, mask_color, 1, debug);
		...
		if (cc.x != 0 || detect_center.RotatedArmCenter.x != 0)
		{
			if (FLAG == 0)
			{
				...
				if (SmallCounter % CalculateSpeedPer == 0)
				{
					...
					if (Clockwise == 0)
					{
						Clockwise = judge_direction(CaliDirectionList);
					}
					else if (Clockwise < 0)
					{
						Clockwise++;
						CaliDirectionList.push_back(RotateSpeed);
					}
					Oldrad = rad;
				}
				...
				if (Clockwise > 0)
				{
					...
					Prediction.x = Prediction.x - 1280 / 2;  //摄像头分辨率w
					Prediction.y = Prediction.y - 1024 / 2;  //摄像头分辨率h
				}
			}
			...
			if (FLAG == 1)
			{
				...
				if (BigCounter % CalculateSpeedPer == 0) //no waitkey150 have waitkey 22
				{
					if (BigCalibrationFlag == 0 || Clockwise <= 0)
					{
						...
						//cout<<"BiasSecond ="<<BiasSecond <<endl;
						if (BiasSecond < 0)
							BiasSecond += 60;
