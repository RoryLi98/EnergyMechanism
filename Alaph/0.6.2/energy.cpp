...
Energy_Agency::Energy_Agency()
{
    ...
    predict_model = cv::ml::SVM::load("/home/t/TODAY/hog_model_small.xml");
    //*********************改svm路径*****************************
}
...
void Energy_Agency::EnergyProcess(Mat &src, Mat &mask_bright, Mat &mask_color, int ColorNumber) //亮度&颜色，在main中被调用 1蓝2红
{
    ...
    if (ColorNumber == 1)
    {
        ...
        energy_agency_color_thresh = 40;  //蓝40 红40
    ...
    resize(mask_bright,mask_bright,Size(mask_bright.cols/2,mask_bright.rows/2),0,0);
