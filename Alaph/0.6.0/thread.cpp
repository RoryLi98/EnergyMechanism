...
//            cout<<distance_1<<":"<<distance_2<<endl;
       ...
       if(circle_center.size()!=0 && GetDistance(circle_center[MinNumber],PreCircleCenter)<50)
       {
           ...
           cout<<"Lock!!!!!!!!!!!!!!!!!!!!!!!!!!!    "<<GetDistance(circle_center[MinNumber],PreCircleCenter)<<endl;
...
int judge_direction(vector<float>list)
{
    ...
    for(int i=0;i<list.size();i++)
        num +=list[i];
    if(num>0) return 1;   //顺时针
    if(num<0) return 2;   //逆时针
    return 0;
}
...
//    bool  SmallCalibrationFlag = 0;
    ...
    while(true)
    {
            while(prdIdx - csmIdx == 0);//cout<<"wait for image\n";
...
//            cout<<"111111"<<endl;
            ee.EnergyProcess(src, mask_bright, mask_color,2);
//            cout<<"222222"<<endl;
...
//            cout<<"444444"<<endl;
    ...
    //        cout<<"CC:"<<cc<<endl;
    //        cout<<"Detect:"<<detect_center.RotatedArmCenter<<endl;
...
//                     cout<<"rad:"<<rad<<endl;
  ...
  //                   cout<<"RotateSpeed"<<RotateSpeed<<endl;
    ...
    //                 Prediction.x = Prediction.x-320-10;
    //                 Prediction.y = Prediction.y-240-35;
                     Prediction.x = Prediction.x-320;
                     Prediction.y = Prediction.y-240;
    ...
    //                 int HeightCal=k*HeightBias+b;
         ...
         //              cout<<"Clockwise Done ="<<Clockwise<<endl;
...
//        sResolution.uSkipMode=0;
       ...
       {
            //TIME_START(produce)
            while(prdIdx - csmIdx >= BUFFER_SIZE);//cout<<"wait for process!!\n";
            ...
            if(CameraGetImageBuffer(hCamera,&sFrameInfo,&pbyBuffer,1000) == CAMERA_STATUS_SUCCESS)
            {
                ...
                resize(matImage, matImage, Size(640, 480));
