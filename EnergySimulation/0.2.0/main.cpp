...
//        else guessnow = int((mark+3.335-abs(now-start))*1000)%3335/1000.0;
        ...
        cout<<0.785*sin(1.884*now)+1.305<<"======"<<0.785*sin(1.884*guessnow-0.8)+1.305<<endl;

        }

        cv::Point2f Prediction=getRotatePoint(clk,perim,cent,((250*cos((471*(now+1))/250)-783*(now+1))/600 + 25/60)*180/CV_PI-((250*cos((471*(now))/250)-783*(now))/600 + 25/60)*180/CV_PI);//18 0
